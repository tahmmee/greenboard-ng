function Timeline($scope, $http, rangeLow, rangeHigh) {
	"use strict";
	...
	$http.get('/timeline',{
		...
		data.forEach(function(build) {
            var weight = build.AbsPassed + build.AbsFailed;
   ...
   $scope.shiftPager = function(dir){
      ...
      if(dir == "left"){
       ...
       Timeline($scope, $http, left, right);
  }
...
app.controller('Timeline', ['$scope', '$http', 
    function ($scope, $http){
        console.log("oklay");
        Timeline($scope, $http, null, null);
}]);
